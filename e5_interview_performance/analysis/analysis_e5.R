...
#setwd("./e5_interview_performance") #go back down to e5_interview_performance directory
...
MakeGroupedBarPlot <- function(data_plot_long) {
    ...
    grouped_bar_plot <- ggplot(data_plot_long, aes(x=plot_names, y=score)) + 
      ...
      xlab("Interview Performance Plots") + ylab("Scaled Rating") + 
...
ArrangeWordClouds <- function() { 
    ...
    Input: none 
    Output: a grid of word clouds (arranged in order of ascending hiring likelihood scores)
    "
...
MakeSentimentBarPlot <- function(data, n_plots, plot_names) {
    "
    Plot the sentiment bar graph in order of ascending hiring likelihood scores. 
    Input: data_long, n_plots, plot_names
    Output: the sentiment bar graph by ascending hiring likelihood scores
    " 
    ...
    sentiment_bar_plot <- ggplot(sentiment_df, aes(x=plot_names, y=mean)) + 
      ...
      geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.2,
                    position=position_dodge(.9)) +
      ggtitle("Mean Sentiment Scores by Ascending Likelihood of Hiring Scores") +
      xlab("Interview Performance Plots") + ylab("Mean Sentiment Score") +
      theme(
...
TopicModeling <- function(dat_long, n_plots, plot_names) { 
    ...
    # Create list of all participant words categorized by interview performance 
    words_raw <- c()
    for(i in 1:n_plots) {
    ...
    topics(words_lda) #assignments of interview performance to each topic
    ...
    arrange_topics <- annotate_figure(arrange_topics, 
                                      left = text_grob("Words", color="black", face ="bold",size=40, rot=90),
                                      bottom = text_grob("Frequency", color="black", face ="bold",size=40,vjust = 0.4))
    print(arrange_topics)
    
    ggsave("topic_model_freq_bar.pdf", arrange_topics, height = 8.5, width = 40)
...
MakePCAFunction <- function(score_features_df) {
    ...
    hiring_likelihood_features <- lmer(data = score_features_df, 
                                       hiring_likelihood ~ PC1 + PC2 + PC3 + PC4 + PC5 + (1 | subject) + (1 | plot_names))
...
CrossValidationAnalysisWtPCs <- function(dat, dat_long, n_ss, n_plots) {
    ...
    x_labs <- ggplot_build(pcs_plot)$layout$panel_params[[1]]$x$get_labels()
...
AnalyzeRidgeRegression <- function(score_features_df) {
    ...
    Input: score_features_df
    Output: Ridge regression results for likelihood of hiring and their plots
    "
    ...
    # Hiring Likelihood
    hiring_likelihood_scores <- score_features_df$hiring_likelihood
    ...
    hiring_likelihood_scores_train <- hiring_likelihood_scores[indeces]
    
    my_predictors_test <- my_predictors[-indeces,]
    hiring_likelihood_scores_test <- hiring_likelihood_scores[-indeces]
    
    # Standardize data 
    my_predictors_train_stdz <- apply(my_predictors_train, 2, scale)
    hiring_likelihood_scores_train_stdz <- scale(hiring_likelihood_scores_train)
    
    my_predictors_test_stdz <- apply(my_predictors_test, 2, scale)
    hiring_likelihood_scores_test_stdz <- scale(hiring_likelihood_scores_test)
    
    # Run regular regression 
    lm_hiring_likelihood_features <- glmnet(my_predictors_train_stdz, hiring_likelihood_scores_train_stdz, 
                                  alpha = 0, lambda = 0)
    
    # Run ridge regression 
    set.seed(123)
    lambdas <- seq(1, 10e-5, length = 100)
    ridge_hiring_likelihood_features <- cv.glmnet(my_predictors_train_stdz, hiring_likelihood_scores_train_stdz, 
                                        nfolds = 10, alpha = 0, lambda = lambdas)
    plot(ridge_hiring_likelihood_features)
    lambda_best <- ridge_hiring_likelihood_features$lambda.min
    ridge_hiring_likelihood_features1 <- glmnet(my_predictors_train_stdz, hiring_likelihood_scores_train_stdz, 
                                      alpha = 0, lambda = lambda_best)
    
    # Order the regression results from most to least important predictors (rounded to 5 digits)
    hiring_likelihood_features_rounded <- round(coef(ridge_hiring_likelihood_features1), 5)
    x_hiring_likelihood_ordered = order(hiring_likelihood_features_rounded@x, decreasing = TRUE) 
    hiring_likelihood_features_rounded@Dimnames[[1]] <- hiring_likelihood_features_rounded@Dimnames[[1]][x_hiring_likelihood_ordered]
    hiring_likelihood_features_rounded@x <- hiring_likelihood_features_rounded@x[x_hiring_likelihood_ordered]
    
    print('hiring likelihood vs. features:')
    print(hiring_likelihood_features_rounded)
    
    # Compare standard regression to ridge regression 
    predict_hiring_likelihood_lm <- predict(lm_hiring_likelihood_features, my_predictors_test_stdz)
    mse_hiring_likelihood_lm <- mean((hiring_likelihood_scores_test_stdz - predict_hiring_likelihood_lm)^2)  
    mse_hiring_likelihood_lm
    predict_hiring_likelihood_ridge <- predict(ridge_hiring_likelihood_features1, my_predictors_test_stdz)
    mse_hiring_likelihood_ridge <- mean((hiring_likelihood_scores_test_stdz - predict_hiring_likelihood_ridge)^2) 
    mse_hiring_likelihood_ridge
    ...
    plot_Fit(hiring_likelihood_scores_test_stdz, predict_hiring_likelihood_lm, my_title = "LM: Hiring Likelihood")
    plot_Fit(hiring_likelihood_scores_test_stdz, predict_hiring_likelihood_ridge, my_title = "RIDGE: Hiring Likelihood")
...
e5_data_clean <- PerformExclusions(e5_data) #num_rows = num_ss
n_after_exclusions <- e5_data_clean$n_after_exclusions[1]
num_subjects_and_plots <- n_after_exclusions*n_plots 

e5_data_long <- Preprocess(e5_data_clean, n_plots, plot_names) #num_rows = num_ss*num_plots [to see e5_data without exclusions, replace e5_data_clean with e5_data]
e5_data_long[, "hiring_likelihood"] <- sapply(e5_data_long[, "hiring_likelihood"], as.numeric) #turn ratings to numeric
...
my_embeddings <- read.csv("data/embeddings_e5.csv", header = TRUE) 
embeddings_avg <- data.frame(embeddings = rowMeans(my_embeddings[2:28])) #create a dataframe 


### (iii) CREATE INTERESTINGNESS DATAFRAME
interestingness <- GetInterestingness(e5_data_long, n_plots)


### (iv) PROCESS FOR PLOTS
e5_data_long <- cbind(e5_data_long, embeddings_avg, interestingness)
e5_data_plot_long <- ProcessForPlots(e5_data_long, n_plots, plot_names) #num_rows = num_plots*num_questions
dim(e5_data_plot_long)
...
grouped_bar_plot <- MakeGroupedBarPlot(e5_data_plot_long) 
...
MakeWordClouds(e5_data_long, n_plots, plot_names) #make word cloud images
...
sentiment_bar_plot <- MakeSentimentBarPlot(e5_data_long, n_plots, plot_names) 
...
topic_modeling <- TopicModeling(e5_data_long, n_plots, plot_names) 
...
e5_dat <- gather(e5_data_long, key = question_type, value = score, hiring_likelihood)
e5_dat <- dplyr::select(e5_dat, subject, plot_names, question_type, score) #rows = num_ss*num_plots*num_questions
e5_sentiment_scores <- CreateSentimentDataframe(e5_data_long, n_plots, plot_names)
e5_dat_final <- cbind(e5_dat, sentiment_score = e5_sentiment_scores[rep(seq_len(nrow(e5_sentiment_scores)), n_after_exclusions), ]$mean)

# Get main statistical effects
main_effects <- GetMainEffects(e5_dat_final, e5_data_long, e5_data_plot_long, data_plot_long, n_plots, plot_names, my_embeddings) 
...
score_features_df <- CreateDataFeaturesDF(e5_data_long, e5_dat_final, features, n_after_exclusions, num_subjects_and_plots) 
...
e5_data_wt_PCs <- MakePCAFunction(score_features_df)


##### (3.3) RUN PREDICTIVE ANALYSES

# Get performance of each predictor and PCA-reduced feature using cross-validation.
cross_validation_analysis_wt_pcs <- CrossValidationAnalysisWtPCs(e5_data_wt_PCs, e5_data_long, n_after_exclusions, n_plots)
...
cross_validation_analysis_wt_predictors <- CrossValidationAnalysisWtPredictors(e5_data_wt_PCs, e5_data_long, n_after_exclusions, n_plots)
...
analysis_files <- list.files(pattern = c("word_analysis_e5.csv|embeddings_e5.csv|correlations_e5.csv"))
